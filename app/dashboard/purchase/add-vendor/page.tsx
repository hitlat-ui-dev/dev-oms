...
export default function AddVendorPage() {
  ...
  const filteredVendors = useMemo(() => {
    ...
    return vendors.filter((v: any) => 
      ...
      String(v.place || "").toLowerCase().includes(q) ||
      ...
      <div className="max-w-6xl mx-auto p-4 space-y-6">
        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => router.back()} 
              className="bg-white border p-3 rounded-2xl text-slate-600 hover:text-[#8b2ef5] hover:shadow-md transition-all active:scale-95"
            >
              <FiArrowLeft size={18} />
            </button>
            <div>
              <h1 className="text-2xl font-black uppercase text-slate-800 tracking-tight">Vendor Management</h1>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Register and manage active suppliers</p>
            </div>
          </div>
        </div>

        {status && (
          <div className="bg-emerald-50 text-emerald-600 p-4 rounded-2xl flex items-center gap-3 font-bold text-sm border border-emerald-100 animate-in fade-in slide-in-from-top-2">
            <FiCheckCircle size={18} /> {status}
          </div>
        )}

        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl space-y-6">
          <div className="flex items-center gap-3 border-b pb-4">
            <div className="p-3 bg-purple-500/10 rounded-2xl text-[#8b2ef5]">
              <FiUserPlus size={22} />
            </div>
            <div>
              <h2 className="font-black uppercase tracking-tight text-slate-800 text-base">New Vendor Entry</h2>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Define business contacts and location</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold text-slate-700 focus:ring-4 focus:ring-purple-500/5 focus:border-[#8b2ef5] transition-all uppercase"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold text-slate-700 focus:ring-4 focus:ring-purple-500/5 focus:border-[#8b2ef5] transition-all"
              ...
              {/* Station/Place */}
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Station / Place</label>
                <div className="relative">
        ...
        <div className="space-y-4 mt-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-slate-200 text-slate-600 px-3 py-1 rounded-full text-xs font-black">{filteredVendors.length}</span>
            </div>
