...
export default function AddSellerPage() {
  ...
  // Separate from `search` (which only covers institute/buyer/bill name and
  // statement descriptions) - this one searches mobile, WhatsApp number and
  // address/place/state, so looking someone up by phone number or locality
  // doesn't get lost among institute-name matches.
  ...
  const sortedSellers = useMemo(() => {
    ...
    let items = [...sellers].filter(s =>
      ...
      (
        !contactQuery ||
        s.mobile?.toLowerCase().includes(contactQuery) ||
        s.whatsappNumber?.toLowerCase().includes(contactQuery) ||
        s.address?.toLowerCase().includes(contactQuery) ||
        s.place?.toLowerCase().includes(contactQuery) ||
        s.state?.toLowerCase().includes(contactQuery)
      )
    );
    if (sortConfig !== null) {
      items.sort((a, b) => {
    ...
    return items;
  }, [sellers, sortConfig, search, contactSearch]);
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <div className="relative flex-1 md:w-64">
                ...
                <input
                  type="text"
                  placeholder="Search mobile / address..."
                  title="Search by mobile number, WhatsApp number, or address/place/state"
                  className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 text-xs font-bold focus:ring-2 focus:ring-blue-500/20 outline-none"
                  value={contactSearch}
